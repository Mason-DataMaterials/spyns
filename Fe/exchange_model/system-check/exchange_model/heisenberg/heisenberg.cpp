...
void initialize(int N, double ** si )
{
    ...
    for (int n = 0; n < N; n++){
        ...
        double theta = M_PI*2.0*(double)dist(engine);
	    double phi = acos( 1 - 2.0*(double)dist(engine));
      
        
        //restrict phi value so z = 1, -1
        for (;;){
            phi = acos( 1 - 2.0*(double)dist(engine));
            double z = cos(phi);
            //cout << n << "\t" << z << "\n";
            if ( abs(z) <=1.01 && abs(z) >= 0.9 ) break;
        }
        cout << n <<":" << phi << "\t" << cos(phi) << "\n";
...
)
{
    ...
    double phi   = si[point][0];
    double theta = si[point][1];
    
    
    double sx  = sin(phi)*cos(new_theta) - sin(phi)*cos(theta);
    double sy =  sin(phi)*sin(new_theta) - sin(phi)*sin(theta);
...
)
{
    ...
    double sx  = sin(new_phi)*cos(theta) - sin(phi)*cos(theta);
    double sy =  sin(new_phi)*sin(theta) - sin(phi)*sin(theta);
    double sz =  cos(new_phi) - cos(phi);
             ...
             double h, double *J, double &M, double &H, double T)
{
    ...
    for (int sweep = 0; sweep < N; sweep++)
    {
        ...
        theta = M_PI*2.0*(double)dist(engine);
...
int main(int argc, char *argv[]){
    ...
    double T, Tmin, Tmax, Tstep;  		                                           //simulation temperature
...
*/
    ...
    if (infile)
    {
        infile>>temp>>temp>>Tmin>>Tstep>>Tmax;
        infile>>temp>>temp>>h;
    ...
    ifile.open("init.txt");
    for (int i =0; i < N; i++){
    ifile << i <<"\t" << 0.0 << "\t" << 0.0 << "\t" << 0.0 <<  "\t" << si[i][0] << "\t" << si[i][1] << "\t" 
