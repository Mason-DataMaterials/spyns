...
void save_data(int N, int MCSteps,double * data, int run)
{
...
int main(int argc, char *argv[]){
   ...
   ifstream infile3; infile3.open("final_config");
   if (infile3.is_open())
    {

   	 for (int i = 0; i < N; i++){
        	infile3 >> si[i][0] >> si[i][1] >> si[i][2] >>  si[i][3] >> si[i][4] >> si[i][5] >> si[i][6] >> si[i][7];
    	}
    }else{
        cout << "file 'final_config' not found for run " << run << "! \n"; 
        return 0; 
   ...
   if ( run = 0 )
   {
    ...
    af <<"aucf_"<<T<<".out" ;
    std::string amf = af.str();
    afile.open(amf.c_str());
    ...
    ofstream ffile; ffile.open("final_config");
    for (int i = 0; i < N; i++){
        ffile << si[i][0] << "\t" << si[i][1] << "\t" 
              << si[i][2] << "\t" << si[i][3] << "\t" << si[i][4] << "\t"
              << si[i][5] << "\t" << si[i][6] << "\t" << si[i][7] << "\n";
