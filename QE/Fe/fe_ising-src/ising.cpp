 ...
 * >>g++ ising.cpp -Wall -lfftw3 -lm -fopenmp -o fe_ising
 * >>./ising
 * 
*/ 
#include <math.h>
...
int main(int argc, char *argv[]){
    ...
    int N = 30;                 //lattice dimension, lenght of single side
    double h = 1.0;             //external magnetic field
    
    double * J = new double [2];
    
    J[0] =2.04731206e-01; J[1] = 6.94636080e-02;  //coupling constants (eV)
    
    
    double T;                //temperature value (Kelvin)
    //double kB = 0.000006334; //boltzmann constant(Ry K^-1)
    double kB = 8.617652e-5; //boltzmann constant(eV K^-1)
    
    int MCSteps = 30000;    //number of Monte Carlo moves
